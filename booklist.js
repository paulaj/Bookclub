$(function() {
	...
	if (Parse.User.current() === null){
		...
		for (var i = 0; i < user.get("goingToRead").length; i++){
			...
			book.first({
				success: function(object){
					$("#goingToRead").append("<a href='"+object.get("url")+".html' class='listedBook'>"+object.get("title")+" by "+object.get("author")+"</a><button type='button' class='currentlyreading' id="+object.get("title")+">Currently Reading</button></br>");
				},
			});
		}
		}
		...
		for (var i = 0; i < user.get("read").length; i++){
			...
			book.first({
				success: function(object){
					$("#alreadyRead").append("<a href='"+object.get("url")+".html' class='listedBook'>"+object.get("title")+" by "+object.get("author")+"</a></br>");
				},
			});
		}
		} 
	
		$(".currentlyreading").click(function(){
			console.log("clicked");
			console.log($(".currentlyreading").id);
		});
